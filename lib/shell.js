 ...
 *  {String} output The output path.
 * }
 * @param {Function} [cb] The callback function.
 */
Shell.unarchive = function(path, options, cb) {
  if (typeof cb === 'function') cb = logError;
  var output = options.output;
  if (!output) return cb('options.output is required');
  var tar = new Tar;
  var basename = Path.basename(options);
  tar.extract(path, options.output, cb);
};
